...
function BuffettArticleInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const initialQ = searchParams.get('q') ?? ''

  const [articles, setArticles] = useState<BuffettArticle[]>([])
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null)
  const [content, setContent] = useState('')
  const [selectedArticle, setSelectedArticle] = useState<BuffettArticle | null>(null)
  ...
  useEffect(() => {
    fetch('/api/buffett-articles?articles=1')
      .then((r) => r.json())
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/buffett-articles?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/buffett-articles?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
      if (data.error) {
        setError(data.error)
        ...
        <div className="shrink-0 px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/buffett-articles?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
...
export default function BuffettArticlePage() {
  return (
