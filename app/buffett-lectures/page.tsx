...
interface BuffettLectureLecture {
  slug: string
  title: string
  date: string
  index: number
...
function BuffettLectureInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const initialQ = searchParams.get('q') ?? ''

  const [lectures, setLectures] = useState<BuffettLectureLecture[]>([])
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null)
  const [content, setContent] = useState('')
  const [selectedLecture, setSelectedLecture] = useState<BuffettLectureLecture | null>(null)
  ...
  useEffect(() => {
    fetch('/api/buffett-lectures?lectures=1')
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data.lectures)) setLectures(data.lectures) })
      .catch(() => {})
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/buffett-lectures?q=${encodeURIComponent(query)}`)
  ...
  const loadLecture = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/buffett-lectures?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
      if (data.error) {
        setError(data.error)
      } else if (!data.lecture || !data.lecture.content) {
        setError('文章无法加载')
      } else {
        setContent(data.lecture.content)
          ...
          {lecturesLoading ? (
            ...
            <div className="mb-1">
              <div className="px-4 py-1.5 text-[10px] font-semibold text-stone-600 uppercase tracking-wider border-b border-stone-800/60 bg-stone-900/30">
                共 {lectures.length} 篇文章
              </div>
              {lectures.map((lecture) => {
                const isSelected = selectedSlug === lecture.slug
                return (
                  <button
                    key={lecture.slug}
                    onClick={() => loadLecture(lecture.slug)}
                  ...
                  >
                    <span className="leading-snug break-words">{lecture.title}</span>
        ...
        <div className="shrink-0 px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/buffett-lectures?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
                ...
                <div key={i} className="rounded-xl border border-stone-700/50 bg-stone-900/60 overflow-hidden">
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-stone-700/40">
                    <span className="text-xs font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded px-2 py-0.5">
                      第 {hit.lecture.index + 1} 篇
                    </span>
                    <span className="text-sm text-stone-300">{hit.lecture.title}</span>
                  ...
                  <div className="px-4 py-3">
                    ...
                    <button
                      onClick={() => loadLecture(hit.lecture.slug)}
...
export default function BuffettLecturePage() {
  return (
