...
function CharlieInner() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/charlie?parts=1').then(r => r.json()),
      fetch('/api/charlie?articles=1').then(r => r.json()),
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/charlie?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/charlie?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
        ...
        <div className="shrink-0 px-3 md:px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/charlie?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!searching && !loading && !error && searchResults.length === 0 && !articleDetail && (
            <div className="flex flex-col items-center justify-center h-full text-center py-24 px-6">
              ...
              <p className="text-stone-600 text-sm">穷查理宝典，共 34 篇</p>
            </div>
          )}
        </div>
      </div>
