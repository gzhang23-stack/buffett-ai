          ...
          return (
            <div key={chapter.chapter}>
              ...
              >
                ...
                <span className="flex-1 truncate leading-snug">{chapter.chapter}</span>
...
function DypInner() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/dyp?chapters=1').then(r => r.json()),
      fetch('/api/dyp?articles=1').then(r => r.json()),
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/dyp?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/dyp?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
        ...
        <div className="shrink-0 px-3 md:px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/dyp?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
