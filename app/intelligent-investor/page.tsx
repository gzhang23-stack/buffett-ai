...
interface IntelligentInvestorChapter {
  slug: string
  title: string
  index: number
}
...
interface SearchResult {
  chapter: IntelligentInvestorChapter
  excerpt: string
  score: number
}
...
function IntelligentInvestorChapterInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const initialQ = searchParams.get('q') ?? ''

  const [chapters, setArticles] = useState<IntelligentInvestorChapter[]>([])
  const [selectedSlug, setSelectedSlug] = useState<string | null>(null)
  const [content, setContent] = useState('')
  const [selectedArticle, setSelectedArticle] = useState<IntelligentInvestorChapter | null>(null)
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)
  const [searchQuery, setSearchQuery] = useState(initialQ)
  const [searchResults, setSearchResults] = useState<SearchResult[]>([])
  const [searching, setSearching] = useState(false)
  const [chaptersLoading, setArticlesLoading] = useState(true)

  useEffect(() => {
    fetch('/api/intelligent-investor?chapters=1')
      .then((r) => r.json())
      .then((data) => { if (Array.isArray(data.chapters)) setArticles(data.chapters) })
      .catch(() => {})
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/intelligent-investor?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/intelligent-investor?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
      if (data.error) {
        setError(data.error)
        ...
        setSelectedArticle(data.chapter)
      }
    } catch {
      setError('加载失败，请检查网络')
    } finally {
            ...
            <div className="mb-1">
              ...
              {chapters.map((chapter) => {
                ...
                return (
                  <button
                    key={chapter.slug}
                    onClick={() => loadArticle(chapter.slug)}
                  ...
                  >
                    <span className="leading-snug break-words">{chapter.title}</span>
        ...
        <div className="shrink-0 px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/intelligent-investor?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
              ...
              {searchResults.map((hit, i) => (
                <div key={i} className="rounded-xl border border-stone-700/50 bg-stone-900/60 overflow-hidden">
                  ...
                  <div className="px-4 py-3">
                    ...
                    <button
                      onClick={() => loadArticle(hit.chapter.slug)}
...
export default function IntelligentInvestorChapterPage() {
  return (
    <div className="h-full overflow-hidden">
      <Suspense fallback={
        ...
        <IntelligentInvestorChapterInner />
      </Suspense>
    </div>
  )
}
