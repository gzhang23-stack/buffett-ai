...
function LiLuInner() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/li-lu?parts=1').then(r => r.json()),
      fetch('/api/li-lu?articles=1').then(r => r.json()),
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/li-lu?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/li-lu?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
        ...
        <div className="shrink-0 px-3 md:px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/li-lu?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
...
export default function LiLuPage() {
  return (
