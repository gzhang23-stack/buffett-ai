...
function MungerDaoInner() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/munger-dao?parts=1').then(r => r.json()),
      fetch('/api/munger-dao?articles=1').then(r => r.json()),
  ...
  const handleSearch = async (q: string) => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/munger-dao?q=${encodeURIComponent(query)}`)
  ...
  const loadArticle = async (slug: string) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/munger-dao?slug=${encodeURIComponent(slug)}`)
      const data = await res.json()
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="w-52 shrink-0 border-r border-stone-800 bg-[#0a0a0a] flex flex-col overflow-hidden">
        <div className="px-4 py-3 border-b border-stone-800 shrink-0">
          ...
          <p className="text-[10px] text-stone-600 mt-1">Poor Charlie&apos;s Almanack</p>
        </div>
        ...
        <div className="shrink-0 px-4 py-3 border-b border-stone-800 bg-[#0f0f0f]">
          <form
            onSubmit={(e) => {
              ...
              if (searchQuery.trim()) router.push(`/munger-dao?q=${encodeURIComponent(searchQuery.trim())}`)
            }}
              ...
              {searchResults.map((hit, i) => (
                <div key={i} className="rounded-xl border border-stone-700/50 bg-stone-900/60 overflow-hidden">
                  <div className="flex items-center gap-2 px-4 py-2.5 border-b border-stone-700/40">
                    ...
                    <span className="ml-auto text-[10px] text-stone-600">{hit.article.year} 年</span>
                  </div>
...
export default function MungerDaoPage() {
  return (
