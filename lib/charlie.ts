...
export interface CharlieArticle {
  slug: string
  index: number
  part_zh: string
  title_zh: string
...
let _articles: CharlieArticle[] | null = null

function loadArticles(): CharlieArticle[] {
  if (_articles) return _articles
  const filePath = path.join(process.cwd(), 'data', 'charlie.json')
  const raw = fs.readFileSync(filePath, 'utf-8')
  _articles = JSON.parse(raw)
  return _articles!
}

export function getAllCharlieArticles(): CharlieArticle[] {
  return loadArticles()
}

export function getCharlieBySlug(slug: string): CharlieArticle | null {
  return loadArticles().find(a => a.slug === slug) ?? null
}

export function getCharlieParts(): { part_zh: string; count: number }[] {
...
export function searchCharlie(query: string, topK = 8) {
  const keywords = query.toLowerCase().split(/\s+/).filter(w => w.length > 0)
  if (keywords.length === 0) return []

  const articles = loadArticles()
  const results: Array<{ article: CharlieArticle; score: number; excerpt: string }> = []
