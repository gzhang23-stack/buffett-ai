...
let _articles: DypArticle[] | null = null

function loadArticles(): DypArticle[] {
  if (_articles) return _articles
  const filePath = path.join(process.cwd(), 'data', 'dyp.json')
  const raw = fs.readFileSync(filePath, 'utf-8')
  _articles = JSON.parse(raw)
  return _articles!
}

export function getAllDypArticles(): DypArticle[] {
  return loadArticles()
}

export function getDypChapters(): { chapter: string; count: number }[] {
...
export function getDypBySlug(slug: string): DypArticle | null {
  return loadArticles().find(a => a.slug === slug) ?? null
}

export function searchDyp(query: string, topK = 8) {
  const keywords = query.toLowerCase().split(/\s+/).filter(w => w.length > 0)
  if (keywords.length === 0) return []

  const articles = loadArticles()
  const results: Array<{ article: DypArticle; score: number; excerpt: string }> = []
