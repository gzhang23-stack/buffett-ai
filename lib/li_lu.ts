...
let _articles: LiLuArticle[] | null = null

function loadArticles(): LiLuArticle[] {
  if (_articles) return _articles
  const filePath = path.join(process.cwd(), 'data', 'li_lu.json')
  const raw = fs.readFileSync(filePath, 'utf-8')
  _articles = JSON.parse(raw)
  return _articles!
}

export function getAllLiLuArticles(): LiLuArticle[] {
  return loadArticles()
}

export function getLiLuBySlug(slug: string): LiLuArticle | null {
  return loadArticles().find(a => a.slug === slug) ?? null
}

export function getLiLuParts(): { part_zh: string; count: number }[] {
...
export function searchLiLu(query: string, topK = 8) {
  const keywords = query.toLowerCase().split(/\s+/).filter(w => w.length > 0)
  if (keywords.length === 0) return []

  const articles = loadArticles()
  const results: Array<{ article: LiLuArticle; score: number; excerpt: string }> = []
