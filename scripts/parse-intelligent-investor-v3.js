...
async function parseDocument() {
  ...
  for (let i = 0; i < chapters.length; i++) {
    ...
    for (let lineIdx = chapter.start + 1; lineIdx < endLine; lineIdx++) {
      ...
      // Skip page headers/footers with more comprehensive patterns
      if (
        // Chapter titles repeated as headers
        line.match(/^第\s*\d+\s*章/) ||
        line.match(/^导言[：:]/) ||
        // Page numbers (standalone numbers)
        line.match(/^\d+$/) ||
        // Very short lines (likely page numbers or artifacts)
        line.length < 10 ||
        // Lines that are just chapter numbers with spaces
        line.match(/^第\s*\d+\s*章\s*$/) ||
        // Lines containing "聪明的投资者" (book title in headers)
        (line.includes('聪明的投资者') && line.length < 30) ||
        // Lines that repeat chapter titles
        (line.includes('投资与投机') && line.length < 50) ||
        (line.includes('投资者与通货膨胀') && line.length < 50) ||
        (line.includes('股市历史') && line.length < 50) ||
        (line.includes('投资组合策略') && line.length < 50) ||
        (line.includes('市场波动') && line.length < 50) ||
        (line.includes('基金投资') && line.length < 30) ||
        (line.includes('投资顾问') && line.length < 50) ||
        (line.includes('证券分析') && line.length < 50) ||
